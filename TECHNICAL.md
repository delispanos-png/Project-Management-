...
- **Οδηγός χρήσης** — μέσα στην εφαρμογή (`projectmanagement/help.js`): 39 κεφάλαια που
  ακολουθούν τη σειρά του μενού, με contextual help ανά οθόνη (`VIEW_TO_HELP`). Κάθε νέα
  οθόνη πρέπει να αποκτά κεφάλαιο **και** εγγραφή στο `VIEW_TO_HELP`.
...
> δυνατότητα. **Ακυρώνει το δικαίωμα που μόλις έδωσες** — δίνεις «Modules» και δεν
> ανοίγει τίποτα. Έχουν αφαιρεθεί δεκάδες τέτοιοι σε τέσσερις γύρους· ο τελευταίος
> βρήκε **31** που είχαν ξεφύγει επειδή ήταν γραμμένοι σε μία γραμμή αντί για τρεις.
> Σήμερα μένουν **πέντε**, όλοι για ενέργειες σε λογαριασμό WHMCS.
>
> Οι έλεγχοι **ιδιοκτησίας** (`!$FULL && $row->assignee !== $adminId`) είναι άλλο
> πράγμα και μένουν: περιορίζουν σε δική σου εγγραφή, δεν ακυρώνουν δικαίωμα.
>
> Έλεγχος:
> ```bash
> # ποιες ενέργειες έχουν σκέτο φραγμό πάνω από τη δυνατότητά τους
> grep -n "if (!\$FULL)" projectmanagement/api.php
> ```
