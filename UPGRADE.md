...
| `mod_cpm_units` | **Τμήματα εργασίας** (Υποστήριξη / Λογιστήριο / Πωλήσεις / E-commerce / Υποδομές / R&D). Το `whmcs_did` γεφυρώνει όσα αντιστοιχούν σε `tblticketdepartments`, ώστε ένα ticket να πέφτει μόνο του στο σωστό τμήμα. |
| `mod_cpm_tasks.unit_id` | Ποιο τμήμα εκτελεί την εργασία. **Η ιεραρχία είναι πελάτης → έργο → εργασίες, και η κάθε εργασία χρεώνεται σε τμήμα** — γι' αυτό το τμήμα κρέμεται στην εργασία, όχι στο έργο. Νέες εργασίες το παίρνουν μόνες τους (από το ticket ή από το `deptid` του project) στο `Db::saveTask()`. |

Όλα δημιουργούνται ιδεμποτικά από το `Db::schema()` — τρέχει μόνο του.

**Πλοήγηση διπλής κατεύθυνσης** (Αύγουστος 2026): πελάτης ⇄ έργο ⇄ εργασία ⇄ τμήμα ⇄ ticket.
Οθόνες `#/units` και `#/unit/<id>`, λωρίδα τμημάτων στην κεφαλίδα του board (`board.meta.unitSplit`),
ψίχουλα στο drawer της εργασίας (`task.owner` / `task.project` / `task.units`), και ομαδοποίηση των
έργων ανά πελάτη στο `#/projects`. **Εξάρτηση:** ο router αναγνωρίζει πλέον αλλαγή id στην ίδια
οθόνη (`S.viewArg`) — χωρίς αυτό το `#/unit/3 → #/unit/5` δεν ξαναφόρτωνε.
