{foreach $sidebar as $item}
    ...
    <div menuItemName="{$item->getName()}" class="item-drop-side {if $item->getClass()}{$item->getClass()}{else}item-drop-side{/if}{if $item->getExtra('mobileSelect') and $item->hasChildren()} hidden-sm hidden-xs{/if}"{if $item->getAttribute('id')} id="{$item->getAttribute('id')}"{/if}>
        ...
        {if $item->hasChildren()}
            <div {if $item->getChildrenAttribute('class')} {$item->getChildrenAttribute('class')}{/if}">
