...
### ΤΙ ΕΓΙΝΕ ΤΕΛΙΚΑ: αναφορές, όχι socket (19/09/2026)

**Το Active Socket ΔΕΝ χρειάστηκε.** Θα ήθελε μόνιμη υπηρεσία που ακούει σε
ανοιχτή θύρα, δηλαδή δύο αλλαγές υποδομής σε μηχάνημα παραγωγής. Το ίδιο
αποτέλεσμα δίνουν δύο αναφορές του XAPI, με ένα απλό ερώτημα ανά ημέρα:

```
ReportInboundCalls/Pbx.GetInboundCalls(periodFrom,periodTo,trunkDns='',callsType=0)
ReportOutboundCalls/Pbx.GetOutboundCalls(...)
```

Μετρημένα: **0,2–0,9΄΄ ανά ημέρα**, ολόκληρη η χρονιά (16.000+ κλήσεις) σε
**~190΄΄**. Τρέχει από το `pulse.php` κάθε 10΄ για τις δύο τελευταίες ημέρες.

**ΤΙ ΔΕΝ ΔΟΥΛΕΥΕΙ — μη το ξαναδοκιμάσεις:**

| Τι | Τι κάνει |
|---|---|
| `GetCallLogData` | 200 αλλά **πάντα 0 γραμμές**. Δοκιμάστηκαν sourceType/destinationType/callsType 0..255, φίλτρα `''` και `null` (το `null` δίνει 400). |
| `GetOldCallLogData` | 405 |
| `CallHistoryView` | Σαρώνει ~123.000 γραμμές. Ακόμη και `$top=1` κάνει timeout στα 25΄΄. **Έριξε την PostgreSQL του PBX** — όλη η ομάδα έμεινε εκτός σύνδεσης. |

**ΤΑ ΔΥΟ ΠΑΓΙΔΙΑ ΤΩΝ ΔΕΔΟΜΕΝΩΝ** (και τα δύο βρέθηκαν με μέτρηση, όχι με
διάβασμα τεκμηρίωσης):

1. **Τα ονόματα πεδίων διαφέρουν ανά αναφορά.** Στις εισερχόμενες το
   `DestinationDn` είναι συχνά κενό — το DN είναι στο `DestinationCallerId`.
   Στις εξερχόμενες ο εξωτερικός αριθμός είναι στο `DestinationCalleeId`
   (όχι `CallerId`) και το `DestinationDn` είναι το trunk.

2. **Μία κλήση = πολλά σκέλη.** Call Script (806) → ουρά υποδοχής (801) →
   χειριστής. Ενώνονται με το `CallHistoryId`. Στις 18/09 ήταν 197 σκέλη για
   96 κλήσεις. Το **Call Script απαντά** και καταγράφεται ως `Answered`: αν
   μετρηθεί, κάθε αναπάντητη φαίνεται απαντημένη.

3. **Το μενού παίρνει δικό του CallHistoryId.** Μια κανονικά απαντημένη κλήση
   άφηνε ΔΥΟ εγγραφές — το μενού (μηδέν ομιλία, κανένας χειριστής) και την
   πραγματική. Μετρήθηκαν **459 τέτοια φαντάσματα στη χρονιά, το 15% όσων
   εμφανίζονταν ως χαμένες**. Ενώνονται όταν υπάρχει άλλη κλήση του ίδιου
   αριθμού μέσα σε 180΄΄· όσα δεν έχουν συνέχεια μένουν, γιατί είναι αληθινές
   εγκαταλείψεις.

Κώδικας: `lib/Pbx3cx/Report.php`. Ενέργειες: `calls_sync` (κουμπί οθόνης, έως
31 ημέρες), `calls_backfill` (έως έναν μήνα ανά αίτημα).

---

### Η αρχική πρόταση: CDR Active Socket (ΔΕΝ ΥΛΟΠΟΙΗΘΗΚΕ)
