...
class Helper
{
    ...
    public static function deleteHetznerResources(Api $api, $serverId, $serviceId)
    {
        ...
        /* Η ΑΝΤΙΣΤΟΙΧΙΣΗ ΦΕΥΓΕΙ ΜΑΖΙ ΜΕ ΤΟΝ SERVER.
           Χωρίς αυτό, ο πίνακας «ποιο VM ανήκει σε ποια υπηρεσία» κρατούσε
           γραμμές για μηχανήματα που είχαν σβηστεί — τρεις τέτοιες βρέθηκαν
           στις 22/09/2026. Λειτουργικά αβλαβείς, αλλά κάθε έλεγχος που ρωτά
           «έκλεισε σωστά η ακύρωση;» τις διάβαζε ως «το VM τρέχει ακόμη» και
           έβγαζε ψευδή συναγερμό.

           Το 404 μετράει κι αυτό ως επιτυχία: αν ο server έχει ήδη φύγει, η
           γραμμή πρέπει να φύγει ακόμη περισσότερο. */
        if ((int) $serviceId > 0 && Capsule::schema()->hasTable('mod_hetzner_instances')) {
            try {
                Capsule::table('mod_hetzner_instances')
                    ->where('service_id', (int) $serviceId)->delete();
            } catch (\Throwable $e) {
                // το νοικοκύρεμα δεν χαλάει ποτέ τη διαγραφή
            }
        }
    }
