...
class Pbx3cxCdr
{
    ...
    public static function e164($n)
    {
        ...
        /* ΠΡΟΘΕΜΑ ΕΞΟΔΟΥ ΓΡΑΜΜΗΣ πριν από το διεθνές 00.
           Ο κατάλογος του 3CX κρατά τους κυπριακούς ως «020035799712702»:
           0 = πάρε γραμμή, 2 = επιλογή διαδρομής, 00357 = διεθνές + Κύπρος.
           Χωρίς αυτό έβγαινε «+020035799712702» — ανύπαρκτος αριθμός που δεν
           ταίριαζε ΠΟΤΕ με τις κλήσεις, κι ας υπήρχε το όνομα στον κατάλογο.
           Γι' αυτό οι μεγαλύτεροι «άγνωστοι» καλούντες ήταν όλοι Κύπρος. */
        if (preg_match('/^0\d?(00\d{8,})$/', $d, $mPfx)) { $d = $mPfx[1]; }
        ...
        /* Ελληνικός: 10 ψηφία. 2 σταθερό · 6 κινητό · 8 ατελής/μεριζόμενης
           χρέωσης (800, 801, 807…) · 9 πολλαπλής χρέωσης. Χωρίς το 8 και το 9,
           τα 800xxxxxxx έμεναν γυμνά και δεν ταύτιζαν ποτέ πελάτη. */
        if (strlen($d) === 10 && strpos('2689', $d[0]) !== false) { return '+30' . $d; }
