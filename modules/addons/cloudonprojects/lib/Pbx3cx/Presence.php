...
class Pbx3cxPresence
{
     ...
     * σε κάθε σφυγμό.
     */
    /** Τι από τα ΑΥΤΟΜΑΤΑ περνά στο τηλέφωνο: manual = τίποτα, all = και Λείπω/Εκτός
     *  από αδράνεια. (Το «meeting» κρατιέται ως αποδεκτή τιμή για παλιές ρυθμίσεις·
     *  η σύσκεψη πάει πλέον πάντα — δες sync().) */
    ...
    public static function sync($adminId, array $pr)
    {
        ...
        $mode = self::mode();
        $want = null;
        /* Η ΣΥΣΚΕΨΗ ΤΟΥ ΗΜΕΡΟΛΟΓΙΟΥ ΕΙΝΑΙ ΔΗΛΩΣΗ, ΟΧΙ ΑΥΤΟΜΑΤΟ (23/09/2026).
           Έμενε πίσω από τη ρύθμιση `presence_auto`, που έχει προεπιλογή «manual» —
           οπότε στην πράξη δεν έφτανε ποτέ στο κέντρο: σύσκεψη 15:00–17:00 και το
           τηλέφωνο χτυπούσε κανονικά. Όμως μια σύσκεψη που ΑΠΟΔΕΧΤΗΚΕΣ είναι δική
           σου πράξη, ακριβώς όπως το να δηλώσεις «σε σύσκεψη» με το χέρι — και η
           μία δήλωση κατέβαζε το τηλέφωνο ενώ η άλλη όχι. Η ρύθμιση μένει για ό,τι
           είναι όντως αυτόματο: το «Λείπω»/«Εκτός» από αδράνεια της εφαρμογής. */
        if ($isManual || $isMeeting) { $want = $isMeeting ? 'meeting' : $status; }
