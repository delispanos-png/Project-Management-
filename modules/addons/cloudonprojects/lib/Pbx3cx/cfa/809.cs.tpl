   ...
   public class Main : ScriptBase<Main>, ICallflow, ICallflowProcessor
   {
      ...
      /* ── CloudOn: ΜΕΤΑ ΤΗΝ ΑΝΑΠΑΝΤΗΤΗ ΟΥΡΑ (809) ──────────────────────────────
         ...
         στο 3CX, θα ξαναγραφτεί. Ωράρια και αργίες ίδια με την AI ρεσεψιόν:
           γραφείο   Δευ–Παρ 09:00–17:29  → Support ({{Q_SUPPORT}}) → CloudOn ({{Q_CLOUDON}}) → «κατειλημμένοι, 1 για επανάκληση» → AI {{Q_CB}}
           απόγευμα  Δευ–Παρ 17:30–19:59, Σάβ 09:30–13:59 → Emergency ({{Q_EMERG}}) → «κατειλημμένοι, 1 για επανάκληση» → AI {{Q_CB}}
