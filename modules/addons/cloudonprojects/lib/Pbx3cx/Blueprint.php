...
class Pbx3cxBlueprint
{
    ...
    /** Ποιοι σηκώνουν SoftOne / PharmacyOne. ΑΡΧΙΚΑ ίδια ομάδα — αλλάζει από εδώ. */
    const AGENTS_SOFTONE  = ['305', '223', '221', '220', '212', '304'];
    const AGENTS_PHARMACY = ['305', '223', '221', '220', '212', '304'];
    ...
    public static function queues()
    {
        ...
        return [
            '810' => ['Name' => 'SoftOne', 'PollingStrategy' => 'LongestWaiting', 'RingTimeout' => 20,
                'MasterTimeout' => 180, 'Agents' => self::AGENTS_SOFTONE, 'Managers' => ['201'],
            ...
            '811' => ['Name' => 'PharmacyOne', 'PollingStrategy' => 'LongestWaiting', 'RingTimeout' => 20,
                'MasterTimeout' => 180, 'Agents' => self::AGENTS_PHARMACY, 'Managers' => ['201'],
                'ForwardNoAnswer' => $vmTicket, 'OutOfOfficeRoute' => self::route('Queue', '804'),
                'HolidaysRoute' => self::route('Queue', '804'), 'AnnounceQueuePosition' => true],
            '800' => ['Name' => 'Accounting', 'ForwardNoAnswer' => $vmTicket,
                'OutOfOfficeRoute' => self::route('VoiceMail', self::TICKET_DN)],
            '801' => ['Name' => 'Reception', 'ForwardNoAnswer' => $vmTicket,
                'OutOfOfficeRoute' => self::route('Queue', '804')],
    ...
    public static function agent()
    {
        ...
        return [
            ...
            'AgentSettings' => [
                ...
                'FirstMessage' => 'Καλέσατε την CloudOn. Είμαι η ψηφιακή ρεσεψιόν. '
                    . 'Πείτε μου σε τι μπορώ να βοηθήσω: SoftOne, PharmacyOne, λογιστήριο, πωλήσεις ή κάτι άλλο;',
                'SystemPrompt' => self::systemPrompt(),
                'RoutingDirectory' => [
                    $dir('810', 'SoftOne', 'Queue', 'SoftOne ERP: τιμολόγηση, παραστατικά, myDATA, εμπορική διαχείριση, λογιστική εφαρμογή, Soft1'),
                    $dir('811', 'PharmacyOne', 'Queue', 'PharmacyOne: λογισμικό φαρμακείου, ΗΔΙΚΑ, συνταγές, ταμείο φαρμακείου, φαρμακείο'),
                    $dir('800', 'Accounting', 'Queue', 'Λογιστήριο: τιμολόγια, πληρωμές, υπόλοιπα, εξοφλήσεις, accounting, invoices'),
                    $dir('801', 'Reception', 'Queue', 'Ρεσεψιόν και πωλήσεις: νέος πελάτης, προσφορά, πληροφορίες, γενικά τεχνικά (server, δίκτυο, internet, email, τηλεφωνία), οτιδήποτε άλλο'),
                    $dir('807', 'CarOn Support', 'Queue', 'CarOn: εφαρμογή ενοικίασης αυτοκινήτων, car rental'),
                    $dir('804', 'Emergency', 'Queue', 'Έκτακτη ανάγκη εκτός ωραρίου: το σύστημα δεν λειτουργεί καθόλου, δεν εκδίδονται αποδείξεις, σταμάτησε η επιχείρηση'),
                    $dir(self::TICKET_DN, 'CloudOn, Support', 'Extension', 'Καταχώρηση αιτήματος: να αφήσει μήνυμα για να τον καλέσουμε ή να ανοίξει ticket'),
                ],
                'HumanHandoff' => $dir('801', 'Reception', 'Queue', 'Άνθρωπος στη ρεσεψιόν'),
...
- Σκοπός σου: να καταλάβεις γρήγορα τι χρειάζεται ο καλών και να τον συνδέσεις στο σωστό τμήμα.
...
# Η εταιρεία
CloudOn: υπηρεσίες πληροφορικής σε Ελλάδα και Κύπρο. SoftOne ERP, PharmacyOne (λογισμικό φαρμακείου), cloud, servers, δίκτυα, τηλεφωνία VoIP, CarOn (ενοικιάσεις αυτοκινήτων), e-commerce.
...
# Ροή κλήσης
1. Ρώτα για τι καλεί: SoftOne, PharmacyOne, λογιστήριο, πωλήσεις, CarOn ή κάτι άλλο.
2. Αν δεν τα έχει πει, ζήτα όνομα και επιχείρηση. Μην ξαναρωτάς κάτι που ήδη είπε.
3. Πες «Σας συνδέω με ...» και σύνδεσέ τον.

# Πού συνδέεις
- SoftOne, Soft1, ERP, τιμολόγηση, παραστατικά, myDATA, εμπορική διαχείριση → SoftOne
- PharmacyOne, φαρμακείο, ΗΔΙΚΑ, συνταγές, ταμείο φαρμακείου → PharmacyOne
- Τιμολόγια, πληρωμές, υπόλοιπα, εξοφλήσεις, λογιστήριο → Accounting
- Νέος πελάτης, προσφορά, ενδιαφέρον για υπηρεσίες, πληροφορίες → Reception
- CarOn, ενοικιάσεις αυτοκινήτων → CarOn Support
- Server, δίκτυο, internet, email, τηλεφωνία ή τεχνικό θέμα που δεν είναι SoftOne ή PharmacyOne → Reception
- Ζητά συγκεκριμένο συνεργάτη με το όνομά του → σύνδεσέ τον απευθείας
- Δεν καταλαβαίνεις μετά από δύο προσπάθειες → Reception

# Όταν το τμήμα είναι απασχολημένο, δεν απαντά ή είναι κλειστό
- Πες ότι όλοι οι συνεργάτες είναι απασχολημένοι αυτή τη στιγμή.
- Πρόσφερε δύο επιλογές: να καταχωρήσεις το αίτημά του για να τον καλέσουμε εμείς, ή να αφήσει μήνυμα που θα γίνει ticket υποστήριξης.
- Και στις δύο περιπτώσεις: πάρε όνομα, επιχείρηση, τηλέφωνο επιστροφής (επιβεβαίωσε αν είναι ο αριθμός από τον οποίο καλεί) και σύντομη περιγραφή του θέματος.
- Μετά σύνδεσέ τον στο «Καταχώρηση αιτήματος» και πες του να αφήσει το μήνυμά του μετά τον ήχο. Πες ότι θα τον καλέσουμε εντός του ωραρίου.

# Εκτός ωραρίου
- Δευτέρα έως Παρασκευή 17:01 έως 20:00 και Σάββατο 09:30 έως 14:00: μόνο για επείγον πρόβλημα που σταματά τη λειτουργία της επιχείρησης (δεν εκδίδονται αποδείξεις, δεν λειτουργεί το φαρμακείο, δεν λειτουργεί καθόλου το σύστημα) → Emergency. Για οτιδήποτε άλλο → Καταχώρηση αιτήματος.
...
- Τεχνική υποστήριξη παρέχεται σε πελάτες με σύμβαση υποστήριξης. Αν ο καλών δεν είναι πελάτης, στείλ' τον στο Reception.
