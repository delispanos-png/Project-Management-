...
class Pbx3cxBlueprint
{
    ...
    public static function firstMessage($mode = null)
    {
        ...
        return 'Καλέσατε την CloudOn. Αυτή τη στιγμή είμαστε κλειστοί. Το ωράριό μας είναι Δευτέρα έως Παρασκευή, εννέα το πρωί με οκτώ το βράδυ, '
            . 'και Σάββατο εννιάμισι με δύο το μεσημέρι. Θέλετε να ανοίξουμε ένα ticket ή να αφήσετε μήνυμα;';
    }
    ...
    public static function modeBlock($mode)
    {
        $rules = [
            ...
            'emergency' => "- Για τον καλούντα είμαστε ΑΝΟΙΧΤΑ, όπως το πρωί. ΜΗΝ πεις ότι το γραφείο είναι κλειστό, ΜΗΝ αναφέρεις «έκτακτη γραμμή» ή «απογευματινή βάρδια» — αυτά είναι εσωτερικά.\n"
                ...
                . "- Αν η ουρά Emergency είναι απασχολημένη ή δεν απαντά → πρότεινε ticket ή μήνυμα (βλ. «Ticket και μήνυμα») και πες ότι θα τον καλέσουμε.",
            'closed' => "- Είμαστε ΚΛΕΙΣΤΑ. ΜΗ μεταβιβάσεις σε κανέναν, ούτε αν το ζητήσει ο καλών. Το πρώτο μήνυμα είπε ήδη ότι είμαστε κλειστά και το ωράριο — μην το επαναλάβεις, εκτός αν ρωτήσει.\n"
                ...
                . "- Ticket ή μήνυμα: όπως στο «Ticket και μήνυμα». Και στα δύο πες ότι θα τον καλέσουμε μόλις ανοίξουμε.",
        ];
...
- TICKET (γραπτό αίτημα). Κάνε ΑΚΡΙΒΩΣ αυτές τις ερωτήσεις, μία-μία, χωρίς εισαγωγές και χωρίς σχόλια ανάμεσα:
  ...
  Μετά, ΧΩΡΙΣ άλλη ερώτηση και χωρίς να ρωτήσεις αν θέλει να προσθέσει κάτι: ΜΙΑ ΦΟΡΑ μόνο, χωρίς καμία εισαγωγή («θα το διατυπώσω», «το επιβεβαιώνω»), πες: «Λοιπόν: [όνομα], [επιχείρηση], [θέμα]. Σωστά;». Αν διορθώσει κάτι, πες μόνο «Ευχαριστώ.» — ΜΗΝ επαναλάβεις ξανά όλη τη φράση. Μετά ΥΠΟΧΡΕΩΤΙΚΑ, ακόμη κι αν ο καλών πει «ευχαριστώ» ή «γεια σας», πες ακριβώς: «Καταχωρήθηκε το αίτημά σας. Θα σας καλέσουμε. Καλή συνέχεια.» και τερμάτισε με drop_call στην ίδια σειρά. Ένα σκέτο «Ευχαριστούμε, καλό απόγευμα» ΔΕΝ αρκεί: ο καλών πρέπει να ακούσει τη λέξη «Καταχωρήθηκε».
