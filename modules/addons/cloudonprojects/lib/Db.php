...
class Db
{
    ...
    public static function install()
    {
        ...
        if (!$s->hasColumn('mod_cpm_interactions', 'offer_id')) {
            $s->table('mod_cpm_interactions', function ($t) {
                $t->integer('offer_id')->unsigned()->nullable()->index();
            });
        }
        ...
        /* ── Τμήμα ανά εργασία ───────────────────────────────────────────────
           Το έργο ανήκει στον ΠΕΛΑΤΗ· η κάθε εργασία του ανατίθεται σε ένα
           DEPARTMENT — πού απευθύνεται το αίτημα. Τα departments είναι
           αυτά του WHMCS (`tblticketdepartments`), τα ίδια που βλέπει ο πελάτης
           όταν ανοίγει ticket· δεν κρατάμε δεύτερο μητρώο.
        ...
        if ($s->hasTable('mod_cpm_units')) {
            if ($s->hasColumn('mod_cpm_tasks', 'unit_id')) {
                foreach (Capsule::table('mod_cpm_units')->whereNotNull('whmcs_did')
                             ->get(['id', 'whmcs_did']) as $u) {
                    Capsule::table('mod_cpm_tasks')->where('unit_id', $u->id)
                        ->whereNull('dept_id')->update(['dept_id' => (int) $u->whmcs_did]);
                }
                $s->table('mod_cpm_tasks', function ($t) { $t->dropColumn('unit_id'); });
            }
            $s->drop('mod_cpm_units');
        }
        /* Όσες εργασίες δεν έχουν ακόμη department το παίρνουν από το ticket
           τους ή από το project τους. */
        if (Capsule::table('mod_cpm_tasks')->whereNull('dept_id')->exists()) {
            foreach (Capsule::table('mod_cpm_tasks')->whereNull('dept_id')
                         ->get(['id', 'ticketid', 'project_id']) as $t) {
                $did = $t->ticketid
                    ? Capsule::table('tbltickets')->where('id', $t->ticketid)->value('did') : null;
                if (!$did && $t->project_id) {
                    $did = Capsule::table('mod_cpm_projects')->where('id', $t->project_id)->value('deptid');
                }
                if ($did) {
                    Capsule::table('mod_cpm_tasks')->where('id', $t->id)->update(['dept_id' => (int) $did]);
                }
            }
        }
        ...
        /* Ώρα έναρξης/λήξης εργασίας. Οι ημερομηνίες μένουν DATE όπως ήταν —
           ...
           την ημέρα· αυτό χρειάζεται ο έλεγχος διαθεσιμότητας. */
        foreach (['start_time', 'due_time'] as $tc) {
            if ($s->hasTable('mod_cpm_tasks') && !$s->hasColumn('mod_cpm_tasks', $tc)) {
                $s->table('mod_cpm_tasks', function ($t) use ($tc) { $t->time($tc)->nullable(); });
            }
        }
        ...
        if (!$s->hasTable('mod_cpm_phone_skip')) {
            $s->create('mod_cpm_phone_skip', function ($t) {
                $t->increments('id');
                $t->string('e164', 24)->unique();
                $t->string('label', 80)->nullable();          // «ΔΕΗ», «τηλεπωλήσεις»…
                $t->integer('admin_id')->unsigned()->nullable();
                $t->dateTime('created_at')->nullable();
            });
        }
        ...
        /* ── Προσφορές με δομημένη κοστολόγηση ──
           Η προσφορά PharmacyOne δεν είναι ένα ποσό: είναι μια **ρύθμιση**
           (users, υποκαταστήματα, modules, εκπτώσεις) από την οποία βγαίνει το
           ποσό και το έγγραφο. Κρατάμε τη ρύθμιση ώστε να ξανανοίγει και να
           αναθεωρείται, αντί να ξαναγράφεται από την αρχή. */
        foreach (['kind' => 'str', 'config' => 'text'] as $col => $kind) {
            if ($s->hasTable('mod_cpm_offers') && !$s->hasColumn('mod_cpm_offers', $col)) {
                $s->table('mod_cpm_offers', function ($t) use ($col, $kind) {
                    $kind === 'text' ? $t->text($col)->nullable()
                                     : $t->string($col, 20)->default('plain');
                });
            }
        }
        /* ── Παράπονα πελατών ──
        ...
        /* ── Χρεώνεται ο χρόνος αυτής της εργασίας; ──────────────────────────
           Το κουτάκι «Χρεώσιμο» έπαιρνε πάντα προεπιλογή «ναι, αφού υπάρχει πελάτης»
           και ΞΑΝΑΓΥΡΝΟΥΣΕ σε «ναι» σε κάθε ξαναχτίσιμο της καρτέλας: ο χειριστής το
           ξετσέκαρε και δεν έμενε ποτέ (αναφορά 15/9/2026). Η απόφαση ανήκει στην
           ΕΡΓΑΣΙΑ — υπάρχει δουλειά που δεν χρεώνεται ποτέ (καλή θέληση, δικό μας
           λάθος, καλυμμένη από συμβόλαιο). NULL = αυτόματο (πελάτης → ναι). */
        if (!$s->hasColumn('mod_cpm_tasks', 'bill_default')) {
            $s->table('mod_cpm_tasks', function ($t) {
                $t->tinyInteger('bill_default')->nullable();   // null=αυτόματο, 0=ποτέ, 1=πάντα
            });
        }
        ...
        /* ΠΟΥ ΑΝΗΚΕΙ Ο ΧΡΟΝΟΣ ΤΗΣ ΚΛΗΣΗΣ. Ο χρόνος ομιλίας μετριέται μόνος του,
           αλλά χωρίς απόδοση είναι ανώνυμος όγκος: ξέρεις ότι μίλησε δύο ώρες,
           δεν ξέρεις σε τι. Ο πελάτης υπάρχει ήδη στην κλήση· λείπουν η εργασία
           και το έργο, και το «τι ήταν» (category — υπήρχε αλλά κενό σε 15.986). */
        if ($s->hasTable('mod_cpm_calls') && !$s->hasColumn('mod_cpm_calls', 'task_id')) {
            $s->table('mod_cpm_calls', function ($t) {
                $t->integer('task_id')->unsigned()->nullable()->index();
                $t->integer('project_id')->unsigned()->nullable()->index();
            });
        }
    ...
    public static function saveTask($id, array $data, $adminId = null)
    {
        ...
        /* Κάθε εργασία ανατίθεται σε ένα department. Αν δεν δηλώθηκε, τη
           βγάζουμε από το ticket που τη γέννησε — αλλιώς από το department του
           project. Χωρίς αυτό μένει «αζήτητη» και δεν τη μετράει καμία ομάδα. */
