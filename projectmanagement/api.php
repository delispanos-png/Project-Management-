...
function cnp_area_defs()
{
    ...
    return [
        ...
        /* ΤΗΛΕΦΩΝΙΚΟ ΚΕΝΤΡΟ (20/09/2026). Τα δικαιώματα `comms.*` υπήρχαν και
           ελέγχονταν, αλλά ΔΕΝ εμφανίζονταν πουθενά στην οθόνη δικαιωμάτων:
           η περιοχή «comms» δεν ήταν δηλωμένη εδώ, οπότε κόβονταν σιωπηλά.
           Αποτέλεσμα: ολόκληρο το κύκλωμα του κέντρου δεν μπορούσε να δοθεί σε
           καμία ομάδα — το έβλεπαν μόνο οι πλήρεις διαχειριστές. */
        'comms'    => ['Τηλεφωνικό κέντρο', 'Κατάλογος, κλήσεις, δρομολόγηση, κίνηση πελάτη, διασύνδεση 3CX'],
...
{
    /* ΤΑ ΧΡΩΜΑΤΑ ΕΙΝΑΙ ΤΟΥ 3CX, ΜΕΤΡΗΜΕΝΑ. Δεν διαλέχτηκαν «να μοιάζουν»:
       διαβάστηκαν pixel-pixel από τις κουκκίδες του client (20/09/2026), ώστε
       η ίδια κατάσταση να έχει ΤΟ ΙΔΙΟ χρώμα στα δύο προγράμματα και να την
       αναγνωρίζεις χωρίς να διαβάσεις.
       Το «σε σύσκεψη» παίρνει το χρώμα του «μην ενοχλείτε», γιατί αυτό ακριβώς
       γίνεται στο τηλέφωνο. */
    return [
        'online'  => ['Διαθέσιμος',    '#36B213', false, true],
        'away'    => ['Λείπω',         '#FF9A0C', false, true],
        'dnd'     => ['Μην ενοχλείτε', '#E05B4A', true,  true],
        'lunch'   => ['Διάλειμμα',     '#2FBBB3', true,  true],
        'trip'    => ['Εκτός έδρας',   '#1D8FD2', false, true],
        'meeting' => ['Σε σύσκεψη',    '#E05B4A', true,  false],
...
/**
 * Ανοιχτά υπόλοιπα ανά πελάτη — ΟΛΑ τα ανεξόφλητα (όχι μόνο τα ληξιπρόθεσμα), με
 * το πραγματικό ανοιχτό ποσό ανά παραστατικό (αξία − πληρωμές − πίστωση), όπως
 * στις Αναστολές. $cid > 0 = μόνο ένας πελάτης.
 * @return array<int,array> clientId => ['open','overdue','days','invs'=>[…],'badDue']
 */
function cnp_open_balances($cid = 0)
{
    $today0 = strtotime('today');
    $allocQ = Capsule::table('tblaccounts')->where('invoiceid', '>', 0)
        ->selectRaw('invoiceid, SUM(amountin) - SUM(CASE WHEN refundid > 0 THEN amountout ELSE 0 END) p')->groupBy('invoiceid');
    if ($cid) { $allocQ->where('userid', $cid); }
    $alloc = [];
    foreach ($allocQ->get() as $r) { $alloc[(int) $r->invoiceid] = (float) $r->p; }
    $ivQ = Capsule::table('tblinvoices')->whereIn('status', ['Unpaid', 'Overdue', 'Payment Pending', 'Collections'])
        ->orderBy('duedate');
    if ($cid) { $ivQ->where('userid', $cid); }
    $out = [];
    foreach ($ivQ->get(['id', 'userid', 'invoicenum', 'date', 'duedate', 'status', 'subtotal', 'tax', 'tax2', 'credit']) as $iv) {
        $due = (string) $iv->duedate;
        $badDue = false;
        if ($due === '' || strpos($due, '0000') === 0) { $due = (string) $iv->date; }
        if ((int) substr($due, 0, 4) < 2000) { $badDue = true; $due = (string) $iv->date; }
        if ($due === '' || strpos($due, '0000') === 0) { continue; }
        $gross = round((float) $iv->subtotal + (float) $iv->tax + (float) $iv->tax2, 2);
        $paid = round(($alloc[(int) $iv->id] ?? 0) + (float) $iv->credit, 2);
        $open = round($gross - $paid, 2);
        if ($open <= 0.5) { continue; }
        $days = (int) floor(($today0 - strtotime($due)) / 86400);   // αρνητικό = δεν έληξε ακόμη
        $c = (int) $iv->userid;
        if (!isset($out[$c])) { $out[$c] = ['open' => 0.0, 'overdue' => 0.0, 'days' => -9999, 'oldestDue' => null, 'invs' => [], 'badDue' => false]; }
        $out[$c]['open'] += $open;
        if ($days > 0) {
            $out[$c]['overdue'] += $open;
            if ($out[$c]['oldestDue'] === null || strtotime($due) < strtotime($out[$c]['oldestDue'])) { $out[$c]['oldestDue'] = $due; }
        }
        $out[$c]['days'] = max($out[$c]['days'], $days);
        $out[$c]['badDue'] = $out[$c]['badDue'] || $badDue;
        $out[$c]['invs'][] = ['id' => (int) $iv->id, 'num' => (string) ($iv->invoicenum ?: $iv->id),
            'issued' => (string) $iv->date, 'due' => $due, 'days' => $days, 'status' => (string) $iv->status,
            'gross' => $gross, 'paid' => $paid, 'open' => $open, 'badDue' => $badDue];
    }
    foreach ($out as &$o) { $o['open'] = round($o['open'], 2); $o['overdue'] = round($o['overdue'], 2); }
    unset($o);
    return $out;
}

/** Γλώσσα επικοινωνίας πελάτη: ό,τι δηλώνει, αλλιώς από τη χώρα. */
function cnp_client_lang($cl, $force = '')
{
    if (in_array($force, ['el', 'en'], true)) { return $force; }
...
function cnp_caps()
{
    ...
    $c = [
        ...
        /* Ο κατάλογος ΔΕΝ είναι πια «του 3CX»: είναι ο δικός μας και η μόνη πηγή
           ταύτισης αριθμών με πρόσωπα. Η ετικέτα έλεγε ακόμη το παλιό. */
        'comms.book'         => ['view',   'Τηλεφωνικός κατάλογος', 'Ποιος είναι πίσω από κάθε αριθμό — πελάτες, προμηθευτές, συνεργάτες'],
        'comms.book.edit'    => ['edit',   'Επεξεργασία καταλόγου', 'Καταχώρηση και αλλαγή επαφών του καταλόγου', 'comms.book'],
        ...
        /* Ήταν `reports.calls`, δηλαδή σε άλλη ενότητα από το υπόλοιπο κέντρο.
           Καμία ομάδα δεν το είχε (μετρήθηκε), οπότε η μετακίνηση δεν αφαιρεί
           πρόσβαση από κανέναν. */
        'comms.calls'        => ['view',   'Τηλεφωνική δραστηριότητα', 'Ποιος μίλησε με ποιον, πόση ώρα, ποιος πελάτης απασχολεί περισσότερο'],
        ...
        'finance.balances'       => ['view',   'Ανοιχτά υπόλοιπα', 'Τι χρωστά κάθε πελάτης — ανά παραστατικό, με υπηρεσίες σε κίνδυνο'],
...
{
    $out = [];
    foreach (Book::resolveMany($nums) as $e => $m) { $out[$e] = $m['name']; }
    return $out;
}
...
function cnp_admin_caps($adminId, $isFull)
{
    ...
    $out = [];
    foreach (cnp_caps() as $k => $v) {
        if (cnp_has_cap($adminId, $isFull, $k)) {
            $out[] = $k;
        }
    }
    return $out;
...
function cnp_action_cap($action)
{
    ...
    if ($map === null) {
        ...
        $add('comms.calls', ['calls_report', 'calls_sync', 'call_drill', 'client_calls']);
        ...
        $add('finance.balances', ['balances', 'balance_reminder']);
...
/**
 * ΕΠΙΣΚΕΠΤΕΣ: ενέργειες που περνούν ΧΩΡΙΣ login επειδή κουβαλούν δικό τους
 * υπογεγραμμένο token (βλ. pm_verify_rsvp / pm_verify_meet στην αρχή του αρχείου).
 * Ο φύλακας δικαιωμάτων τις προσπερνά — σε έναν πελάτη δεν έχει τι να κρίνει.
 * Επιπλέον, όσα ξεκινούν από «rtc_» περνούν με έγκυρο token δωματίου CloudOn Meet.
 */
function cnp_guest_actions()
{
    return ['event_rsvp_public'];
}
...
/* Οι δύο διαδρομές ΧΩΡΙΣ login έχουν ήδη περάσει δικό τους υπογεγραμμένο token
   στην αρχή του αρχείου: ο guest του CloudOn Meet είναι δεμένος στο δωμάτιό του,
...
$needCap = $guestOk ? null : cnp_action_cap($action);
if ($needCap !== null) {
...
} elseif (!$guestOk && !in_array($action, cnp_open_actions(), true)) {
...
case 'templates':
    $tpl = [];
    foreach (Capsule::table('mod_cpm_templates')->orderByDesc('active')->orderBy('name')->get() as $x) {
        $steps = [];
        foreach (Capsule::table('mod_cpm_template_steps')->where('template_id', $x->id)
                     ->orderBy('sort')->orderBy('id')->get() as $st) {
            $steps[] = ['id' => (int) $st->id, 'sort' => (int) $st->sort, 'title' => $st->title,
                'descr' => (string) $st->descr,
                'dept' => $st->dept_id ? (int) $st->dept_id : null,
                'assignee' => $st->assignee ? (int) $st->assignee : null,
                'offStart' => (int) $st->off_start, 'offDue' => (int) $st->off_due,
                'offDeadline' => $st->off_deadline !== null ? (int) $st->off_deadline : null,
                'est' => $st->est_minutes ? (int) $st->est_minutes : null,
                'prio' => (int) $st->prio,
    ...
    out(['templates' => $tpl, 'depts' => cnp_depts(), 'canManage' => $FULL, 'products' => $prods,
        ...
        'admins' => (function () {
            $o = [];
            foreach (Db::admins() as $a) {
                $o[] = ['id' => (int) $a->id, 'name' => trim($a->firstname . ' ' . $a->lastname)];
            }
            return $o;
        })()]);
...
case 'template_save':
    ...
    $tid = (int) ($in['id'] ?? 0);
    if ($tid) {
        Capsule::table('mod_cpm_templates')->where('id', $tid)->update($row);
    } else {
        $row['created_by'] = $adminId;
        $row['created_at'] = date('Y-m-d H:i:s');
        $tid = (int) Capsule::table('mod_cpm_templates')->insertGetId($row);
    }
    out(['ok' => true, 'id' => $tid]);
...
case 'template_step_save':
    ...
    $row = ['template_id' => $tid,
        'title' => mb_substr(trim((string) ($in['title'] ?? '')), 0, 200) ?: 'Βήμα',
        'descr' => mb_substr(trim((string) ($in['descr'] ?? '')), 0, 20000),
        'dept_id' => (int) ($in['dept'] ?? 0) ?: null,
        'assignee' => (int) ($in['assignee'] ?? 0) ?: null,
        'off_start' => max(0, (int) ($in['offStart'] ?? 0)),
        'off_due' => max(0, (int) ($in['offDue'] ?? 0)),
        'off_deadline' => ($in['offDeadline'] ?? '') !== '' ? max(0, (int) $in['offDeadline']) : null,
        'est_minutes' => (int) ($in['est'] ?? 0) ?: null,
        'prio' => min(2, max(0, (int) ($in['prio'] ?? 0))),
        'checks' => trim((string) ($in['checks'] ?? ''))];
    $sid = (int) ($in['id'] ?? 0);
    if ($sid) {
        Capsule::table('mod_cpm_template_steps')->where('id', $sid)->update($row);
    } else {
        $row['sort'] = 1 + (int) Capsule::table('mod_cpm_template_steps')->where('template_id', $tid)->max('sort');
        $sid = (int) Capsule::table('mod_cpm_template_steps')->insertGetId($row);
    }
    out(['ok' => true, 'id' => $sid]);
...
case 'book_list':                        // Ο ΚΑΤΑΛΟΓΟΣ — η λίστα
    /* Ένα ερώτημα, με ό,τι χρειάζεται η οθόνη: ποιος είναι, πόσο μας απασχολεί,
       αν είναι πελάτης, αν έφτασε στα τηλέφωνα. Τα πάντα από ΕΝΑΝ κατάλογο. */
    $bq = trim((string) ($_GET['q'] ?? ''));
    $bStatus = (string) ($_GET['status'] ?? '');
    $bOnly = (string) ($_GET['only'] ?? '');   // '' | client | noclient | nopbx | pbxerr | due
    $bSort = (string) ($_GET['sort'] ?? 'talk');
    ...
    $phones = [];
    if ($ids) {
        foreach (Capsule::table('mod_cpm_book_phones')->whereIn('book_id', $ids)
            ->orderBy('sort')->get() as $p) {
            $phones[(int) $p->book_id][] = ['e164' => $p->e164, 'label' => $p->label];
        }
    }
    /* Πόσο μας απασχολεί ο καθένας: ο λόγος που ο κατάλογος δεν είναι απλώς
       λίστα ονομάτων. Ένα ερώτημα για όλους. */
    $stats = [];
    if ($ids) {
        foreach (Capsule::table('mod_cpm_calls')->whereIn('book_id', $ids)
            ->groupBy('book_id')
            ->selectRaw('book_id, COUNT(*) n, COALESCE(SUM(talk_seconds),0) t, SUM(answered=0) miss')
            ->get() as $s2) {
            $stats[(int) $s2->book_id] = ['calls' => (int) $s2->n, 'talk' => (int) $s2->t,
                'missed' => (int) $s2->miss];
        }
    }
    /* Διπλά τηλέφωνα ανάμεσα σε καρτέλες — για να τα καθαρίσει άνθρωπος. */
    $dups = array_flip(Capsule::table('mod_cpm_book_phones')->groupBy('e164')
        ->havingRaw('COUNT(DISTINCT book_id) > 1')->pluck('e164')->all());
    ...
    foreach ($rows as $r) {
        ...
        $ph = $phones[$id] ?? [];
        $dup = false;
        foreach ($ph as $x) { if (isset($dups[$x['e164']])) { $dup = true; break; } }
        $st = $stats[$id] ?? ['calls' => 0, 'talk' => 0, 'missed' => 0];
        $out[] = [
            'id' => $id, 'kind' => $r->kind,
            'name' => Book::label((array) $r),
            'company' => (string) $r->company, 'title' => (string) $r->title,
            'city' => (string) $r->city, 'tags' => (string) $r->tags,
            'status' => (string) $r->status,
            'phones' => $ph, 'dup' => $dup,
            'client' => $r->clientid ? (int) $r->clientid : 0,
            'clientName' => $r->clientid ? clientLabel((int) $r->clientid) : '',
            'pbx' => $r->pbx_id ? 1 : 0, 'toPbx' => (int) $r->to_pbx,
            'pbxError' => (string) $r->pbx_error,
            'lastCall' => $r->last_call_at, 'nextAt' => $r->next_at, 'nextNote' => (string) $r->next_note,
            'owner' => $r->owner_id ? (int) $r->owner_id : 0,
            'calls' => $st['calls'], 'talk' => $st['talk'], 'missed' => $st['missed'],
        ];
    }
    ...
    out(['items' => $out, 'q' => $bq, 'only' => $bOnly, 'status' => $bStatus, 'sort' => $bSort,
        'counts' => [
...
case 'book_get':                         // Η ΚΑΡΤΕΛΑ
    $gId = (int) ($_GET['id'] ?? 0);
    $g = Capsule::table('mod_cpm_book')->where('id', $gId)->first();
    if (!$g) { fail('Δεν βρέθηκε η καρτέλα'); }

    $gPh = [];
    foreach (Capsule::table('mod_cpm_book_phones')->where('book_id', $gId)->orderBy('sort')->get() as $p) {
        $gPh[] = ['id' => (int) $p->id, 'e164' => $p->e164, 'raw' => (string) $p->raw, 'label' => $p->label];
    }
    /* Τα δικά μας πεδία: ο ορισμός μαζί με την τιμή, ώστε η οθόνη να μη
       χρειάζεται δεύτερο ερώτημα ούτε να ξέρει τι υπάρχει. */
    $gVals = Capsule::table('mod_cpm_book_values')->where('book_id', $gId)->pluck('value', 'field_id')->all();
    $gFields = [];
    foreach (Capsule::table('mod_cpm_book_fields')->where('active', 1)->orderBy('sort')->get() as $f) {
        $gFields[] = ['id' => (int) $f->id, 'key' => $f->fkey, 'label' => $f->label,
            'type' => $f->ftype, 'options' => array_values(array_filter(array_map('trim',
                explode('|', (string) $f->options)))), 'hint' => (string) $f->hint,
            'value' => (string) ($gVals[$f->id] ?? '')];
    }

    /* Το χρονολόγιο: ό,τι έγινε με αυτή την επαφή, σε μία στήλη. */
    $gTl = [];
    foreach (Capsule::table('mod_cpm_interactions')->where('book_id', $gId)
        ->orderBy('happened_at', 'desc')->limit(50)->get() as $i) {
        $gTl[] = ['id' => (int) $i->id, 'kind' => $i->kind, 'summary' => (string) $i->summary,
            'detail' => (string) $i->detail, 'at' => $i->happened_at,
            'who' => $i->admin_id ? Db::adminName((int) $i->admin_id) : '',
            'followup' => $i->followup_date, 'task' => (int) $i->task_id, 'ticket' => (int) $i->ticketid];
    }

    $gCalls = [];
    foreach (Capsule::table('mod_cpm_calls')->where('book_id', $gId)
    ...
    $gAgg = Capsule::table('mod_cpm_calls')->where('book_id', $gId)
        ->selectRaw('COUNT(*) n, COALESCE(SUM(talk_seconds),0) t, SUM(answered=0) miss')->first();
    ...
    out(['card' => [
            ...
            'updatedBy' => $g->updated_by ? Db::adminName((int) $g->updated_by) : '',
        ],
        ...
        'people' => (function () {
            /* Db::admins() επιστρέφει Collection, όχι πίνακα — και οι bot
               λογαριασμοί δεν έχουν θέση σε λίστα υπευθύνων. */
            $o = [];
            foreach (Db::admins() as $a) {
                $nm = trim($a->firstname . ' ' . $a->lastname);
                if (!cnp_is_bot($nm, $a->username ?? '')) { $o[] = ['id' => (int) $a->id, 'name' => $nm]; }
...
case 'book_save':                        // αποθήκευση καρτέλας — ΚΑΙ αποστολή στο 3CX
    $sId = (int) ($in['id'] ?? 0);
    $sCo = mb_substr(trim((string) ($in['company'] ?? '')), 0, 160);
    $sFi = mb_substr(trim((string) ($in['first'] ?? '')), 0, 60);
    $sLa = mb_substr(trim((string) ($in['last'] ?? '')), 0, 60);
    if ($sCo === '' && $sFi === '' && $sLa === '') { fail('Γράψε επωνυμία ή όνομα'); }

    /* Τα τηλέφωνα κανονικοποιούνται ΠΡΙΝ αποθηκευτούν — αλλιώς ο ίδιος αριθμός
       γραμμένος αλλιώς δεν θα ταίριαζε ποτέ με τις κλήσεις. */
    $sPhones = [];
    foreach ((array) ($in['phones'] ?? []) as $p) {
        $raw = trim((string) ($p['raw'] ?? ''));
        if ($raw === '') { continue; }
        $e = Pbx3cxCdr::e164($raw);
        if (strlen(preg_replace('/\D/', '', $e)) < 6) { continue; }
        $lab = (string) ($p['label'] ?? 'main');
        if (!isset(Book::phoneLabels()[$lab])) { $lab = 'other'; }
        $sPhones[$e] = ['label' => $lab, 'raw' => mb_substr($raw, 0, 40)];
    }
    ...
    $row = [
        ...
        'tax_office' => mb_substr(trim((string) ($in['taxOffice'] ?? '')), 0, 60) ?: null,
        'address' => mb_substr(trim((string) ($in['address'] ?? '')), 0, 200) ?: null,
        'city' => mb_substr(trim((string) ($in['city'] ?? '')), 0, 80) ?: null,
        'postcode' => mb_substr(trim((string) ($in['postcode'] ?? '')), 0, 12) ?: null,
        'country' => mb_substr(trim((string) ($in['country'] ?? '')), 0, 40) ?: null,
    ...
    if ($sId) {
        Capsule::table('mod_cpm_book')->where('id', $sId)->update($row);
    } else {
        $row['created_by'] = $adminId; $row['created_at'] = $row['updated_at'];
        $sId = (int) Capsule::table('mod_cpm_book')->insertGetId($row);
    }

    /* Τα τηλέφωνα ξαναγράφονται ολόκληρα: έτσι η αφαίρεση ενός είναι απλώς
       «δεν στάλθηκε», χωρίς ξεχωριστή ενέργεια διαγραφής. */
    $old = Capsule::table('mod_cpm_book_phones')->where('book_id', $sId)->pluck('e164')->all();
    Capsule::table('mod_cpm_book_phones')->where('book_id', $sId)->delete();
    $sort = 0;
    foreach ($sPhones as $e => $p) {
        Capsule::table('mod_cpm_book_phones')->insertOrIgnore(['book_id' => $sId, 'e164' => $e,
            'raw' => $p['raw'], 'label' => $p['label'], 'sort' => $sort++]);
    }

    /* Τα δικά μας πεδία. */
    foreach ((array) ($in['fields'] ?? []) as $fid => $val) {
        $fid = (int) $fid;
        if (!$fid) { continue; }
        $val = mb_substr(trim((string) $val), 0, 4000);
        if ($val === '') { Capsule::table('mod_cpm_book_values')->where('book_id', $sId)->where('field_id', $fid)->delete(); }
        else { Capsule::table('mod_cpm_book_values')->updateOrInsert(
            ['book_id' => $sId, 'field_id' => $fid], ['value' => $val]); }
    }

    /* Κάθε αλλαγή τηλεφώνου αλλάζει ΠΟΙΟΣ φαίνεται σε παλιές κλήσεις. */
    foreach (array_unique(array_merge($old, array_keys($sPhones))) as $e) { Book::reindexCalls($e); }
    Book::refreshLastCall();

    /* Ο κατάλογος φτάνει στο 3CX ΣΤΟ ΠΑΡΑΣΚΗΝΙΟ (απόφαση 20/09/2026, δεύτερη): μόνο
       έτσι η AI ρεσεψιόν χαιρετά τον πελάτη με το όνομά του και τον στέλνει στην ουρά
       του χωρίς ερωτήσεις. Η αποστολή ΔΕΝ μπλοκάρει και ΔΕΝ χαλάει ποτέ την αποθήκευση:
       ό,τι αποτύχει το ξαναστέλνει το βραδινό cron. Η οθόνη δεν ασχολείται. */
    $pbx = null;
...
case 'book_del':
    ...
    foreach ($dNums as $e) { Book::reindexCalls($e); }
    if (function_exists('logActivity')) {
        logActivity('CPM κατάλογος: διαγραφή «' . Book::label((array) $dB) . '» (#' . $dId
            . ') από admin #' . $adminId);
    }
    out(['ok' => true]);

case 'book_note':                        // σημείωση στο χρονολόγιο της καρτέλας
    $nB = (int) ($in['book'] ?? 0);
    if (!Capsule::table('mod_cpm_book')->where('id', $nB)->exists()) { fail('Δεν βρέθηκε η καρτέλα'); }
    $nSum = mb_substr(trim((string) ($in['summary'] ?? '')), 0, 255);
    if ($nSum === '') { fail('Γράψε τι έγινε'); }
    Capsule::table('mod_cpm_interactions')->insert([
        'book_id' => $nB, 'clientid' => Capsule::table('mod_cpm_book')->where('id', $nB)->value('clientid'),
        'kind' => (string) ($in['kind'] ?? 'note'),
        'summary' => $nSum, 'detail' => mb_substr(trim((string) ($in['detail'] ?? '')), 0, 4000) ?: null,
        'admin_id' => $adminId, 'happened_at' => date('Y-m-d H:i:s'),
        'followup_date' => preg_match('/^\d{4}-\d{2}-\d{2}$/', (string) ($in['followup'] ?? ''))
            ? $in['followup'] : null,
        'created_at' => date('Y-m-d H:i:s')]);
    /* Η επόμενη κίνηση ανεβαίνει και στην καρτέλα, για να φαίνεται στη λίστα. */
    if (!empty($in['followup'])) {
        Capsule::table('mod_cpm_book')->where('id', $nB)
            ->update(['next_at' => $in['followup'], 'next_note' => mb_substr($nSum, 0, 200)]);
    }
    out(['ok' => true]);

case 'book_import':                      // γέμισμα από 3CX και WHMCS
    $src = (string) ($in['from'] ?? '');
    $res = [];
    if ($src === 'pbx' || $src === 'both') {
        if (!Pbx3cxClient::configured()) { fail('Δεν έχει ρυθμιστεί η διασύνδεση 3CX'); }
        $res['pbx'] = Book::importFromPbx();
    }
    if ($src === 'whmcs' || $src === 'both') { $res['whmcs'] = Book::importFromWhmcs(); }
    if (!$res) { fail('Διάλεξε πηγή'); }
    Book::reindexCalls();
    Book::refreshLastCall();
    if (function_exists('logActivity')) {
        logActivity('CPM κατάλογος: εισαγωγή (' . $src . ') από admin #' . $adminId
...
case 'book_fields':                      // τα δικά μας πεδία — ορισμός
    if (!empty($in['save'])) {
        if (!cnp_has_cap($adminId, $FULL, 'comms.book.edit')) { fail('Δεν έχεις δικαίωμα', 403); }
        $f = (array) $in['save'];
        $fid = (int) ($f['id'] ?? 0);
        $lbl = mb_substr(trim((string) ($f['label'] ?? '')), 0, 80);
        if ($lbl === '') { fail('Γράψε όνομα πεδίου'); }
        $key = preg_replace('/[^a-z0-9_]/', '', strtolower((string) ($f['key'] ?? '')));
        if ($key === '') { $key = 'f' . substr(md5($lbl . microtime(true)), 0, 10); }
        $data = ['label' => $lbl, 'fkey' => $key,
            'ftype' => in_array(($f['type'] ?? ''), ['text','textarea','number','date','select','check','url'], true)
                ? $f['type'] : 'text',
            'options' => mb_substr(trim((string) ($f['options'] ?? '')), 0, 400) ?: null,
            'hint' => mb_substr(trim((string) ($f['hint'] ?? '')), 0, 160) ?: null,
            'sort' => (int) ($f['sort'] ?? 0), 'active' => empty($f['off']) ? 1 : 0,
            'in_list' => !empty($f['inList']) ? 1 : 0];
        if ($fid) { Capsule::table('mod_cpm_book_fields')->where('id', $fid)->update($data); }
        else { $fid = (int) Capsule::table('mod_cpm_book_fields')->insertGetId($data); }
        out(['ok' => true, 'id' => $fid]);
    }
    if (!empty($in['del'])) {
        if (!cnp_has_cap($adminId, $FULL, 'comms.book.delete')) { fail('Δεν έχεις δικαίωμα', 403); }
        $fid = (int) $in['del'];
        Capsule::table('mod_cpm_book_values')->where('field_id', $fid)->delete();
        Capsule::table('mod_cpm_book_fields')->where('id', $fid)->delete();
        out(['ok' => true]);
    }
    $ff = [];
    foreach (Capsule::table('mod_cpm_book_fields')->orderBy('sort')->get() as $f) {
        $ff[] = ['id' => (int) $f->id, 'key' => $f->fkey, 'label' => $f->label, 'type' => $f->ftype,
            'options' => (string) $f->options, 'hint' => (string) $f->hint, 'sort' => (int) $f->sort,
            'active' => (int) $f->active, 'inList' => (int) $f->in_list,
            'used' => (int) Capsule::table('mod_cpm_book_values')->where('field_id', $f->id)->count()];
    }
    out(['fields' => $ff, 'canEdit' => cnp_has_cap($adminId, $FULL, 'comms.book.edit')]);
...
case 'pbx_sync':                         // ΦΑΣΗ 3 — τράβα τη δομή από το PBX
    if (!Pbx3cxClient::configured()) { fail('Δεν έχει ρυθμιστεί η διασύνδεση'); }
    $sy = Pbx3cxSync::run();
    /* Ο κατάλογος ΔΕΝ διαβάζεται πια από εδώ: τον κρατάμε εμείς και τον
       στέλνουμε προς τα έξω (Σύστημα → Τηλεφωνικός κατάλογος). Αν διαβάζαμε κι
       από τις δύο μεριές, θα κέρδιζε ο τελευταίος που πάτησε αποθήκευση. */
    if (function_exists('logActivity')) {
...
case 'call_link':                        // «αυτό το τηλέφωνο είναι ο πελάτης Χ»
    ...
    /* ΚΑΙ ΣΤΟΝ ΚΑΤΑΛΟΓΟ: ο πελάτης πρέπει να φαίνεται και στη συσκευή του
       συναδέλφου, όχι μόνο σε αυτή την οθόνη. Γράφουμε στον ΔΙΚΟ μας κατάλογο —
       εκείνος ενημερώνει το τηλεφωνικό κέντρο, όχι εμείς απευθείας. */
    $lkBook = null;
    if (!empty($in['toBook']) && cnp_has_cap($adminId, $FULL, 'comms.book.edit')) {
        $lkName = mb_substr(clientLabel($lkClient), 0, 160);
        $lkRow = Capsule::table('mod_cpm_book_phones')->where('e164', $lkE)->value('book_id');
        $nowB = date('Y-m-d H:i:s');
        if ($lkRow) {
            Capsule::table('mod_cpm_book')->where('id', $lkRow)->update([
                'clientid' => $lkClient, 'company' => $lkName,
                'updated_by' => $adminId, 'updated_at' => $nowB]);
            $lkBookId = (int) $lkRow;
            $lkBook = 'updated';
        } else {
            $lkBookId = (int) Capsule::table('mod_cpm_book')->insertGetId([
                'kind' => 'company', 'company' => $lkName, 'status' => 'active',
                'clientid' => $lkClient, 'to_pbx' => 1,
                'created_by' => $adminId, 'updated_by' => $adminId,
                'created_at' => $nowB, 'updated_at' => $nowB]);
            Capsule::table('mod_cpm_book_phones')->insertOrIgnore([
                'book_id' => $lkBookId, 'e164' => $lkE, 'raw' => $lkE, 'label' => 'main', 'sort' => 0]);
            $lkBook = 'created';
        }
...
case 'portfolio':
    ...
    foreach ($src as $p) {
        ...
        $projs[] = ['id' => (int) $p->id, 'name' => $p->name, 'color' => $p->color,
            ...
            /* Οι ιδιωτικές σημειώσεις φεύγουν ΜΟΝΟ στον υπεύθυνο και στους
               διαχειριστές — για τους υπόλοιπους δεν υπάρχει καν το πεδίο. */
...
case 'save_project':
    ...
    /* Η ραχοκοκαλιά: πελάτης → (προϊόν) → τμήμα → έργο. Σε ΝΕΟ έργο πελάτη πελάτης και
...
case 'project_pm_notes':                  // ιδιωτικές σημειώσεις υπεύθυνου έργου
    /* Ξεχωριστή ενέργεια επίτηδες: το `save_project` χτίζει ΟΛΑ τα πεδία από το
       input, οπότε μια κλήση «μόνο για τις σημειώσεις» θα μηδένιζε budget,
       ημερομηνίες, υπεύθυνο και τα υπόλοιπα. */
    $pp = Db::project((int) ($in['id'] ?? 0));
    if (!$pp) { fail('project', 404); }
    if (!$FULL && (int) $pp->manager_id !== $adminId) {
        fail('Μόνο ο υπεύθυνος του έργου κρατάει ιδιωτικές σημειώσεις', 403);
    }
...
case 'ai_summary':
    ...
    $ragTxt = '';
    if ($action === 'ai_suggest' && trim((string) ($in['draft'] ?? '')) === '') {
        try {
    ...
    /* Προσχέδιο του χειριστή: τι θέλει να πει, με δυο λόγια. Όταν υπάρχει, το
       ...
       του πελάτη. Αλλιώς θα «βελτίωνε» προσθέτοντας υποσχέσεις που δεν δώσαμε. */
    $draft = trim((string) ($in['draft'] ?? ''));
    $draftTxt = $draft !== ''
        ? "\n\nΠΡΟΣΧΕΔΙΟ ΤΟΥ ΣΥΝΑΔΕΛΦΟΥ — ΑΥΤΟ ΕΙΝΑΙ ΤΟ ΠΕΡΙΕΧΟΜΕΝΟ ΤΗΣ ΑΠΑΝΤΗΣΗΣ:\n"
            . mb_substr($draft, 0, 2000) . "\n\n"
            . "ΚΑΝΟΝΕΣ ΓΙΑ ΤΟ ΠΡΟΣΧΕΔΙΟ — ΚΡΙΣΙΜΟ:\n"
            . "- Πες ΑΚΡΙΒΩΣ ό,τι λέει το προσχέδιο. Μη προσθέσεις γεγονότα, αιτίες,"
            . " χρόνους, τιμές ή υποσχέσεις που δεν υπάρχουν σε αυτό.\n"
            . "- Μην αφαιρέσεις πληροφορία του προσχεδίου.\n"
            . "- Διόρθωσε ύφος, δομή, ορθογραφία και ευγένεια· απάντησε ρητά σε ό,τι"
            . " ρώτησε ο πελάτης, χρησιμοποιώντας μόνο όσα λέει το προσχέδιο.\n"
            . "- Αν το προσχέδιο δεν καλύπτει κάτι που ρώτησε ο πελάτης, μην το"
            . " εφεύρεις — απλώς μην το θίξεις.\n"
            . "- Αν το προσχέδιο είναι σε άλλη γλώσσα από του πελάτη, μετάφρασέ το"
            . " στη γλώσσα του πελάτη.\n"
            . "- ΜΗΝ κλείσεις με υπογραφή ή όνομα εταιρείας (π.χ. «Best regards,"
            . " CloudOn Support»). Η υπογραφή μπαίνει αυτόματα από το σύστημα.\n"
        : '';
...
/* ================= ΑΝΟΙΧΤΑ ΥΠΟΛΟΙΠΑ & ΥΠΕΝΘΥΜΙΣΕΙΣ ΕΞΟΦΛΗΣΗΣ =================
   Η οθόνη «Αναστολές» ξεκινά από την υπηρεσία που πρέπει να πέσει. Εδώ ξεκινάμε
   από τον ΠΕΛΑΤΗ και το τι χρωστά — και το εργαλείο είναι μια ευγενική υπενθύμιση,
   ΠΡΙΝ φτάσουμε στην αναστολή. Ίδια αριθμητική με τις Αναστολές (cnp_open_balances). */
case 'balances':
    $graceB = (int) (Capsule::table('tblconfiguration')->where('setting', 'AutoSuspensionDays')->value('value') ?: 10);
    $bal = cnp_open_balances();
    if (!$bal) { out(['rows' => [], 'grace' => $graceB, 'sum' => ['clients' => 0, 'open' => 0, 'overdue' => 0, 'risk' => 0, 'notDue' => 0]]); }
    $cids = array_keys($bal);
    $clients = [];
    foreach (Capsule::table('tblclients')->whereIn('id', $cids)->get(['id', 'firstname', 'lastname', 'companyname', 'email', 'language', 'country', 'status']) as $c) {
        $clients[(int) $c->id] = $c;
    }
    /* Τελευταία υπενθύμιση & τελευταία ειδοποίηση αναστολής ανά πελάτη. */
    $lastRem = [];
    foreach (Capsule::table('mod_cpm_pay_reminders')->whereIn('client_id', $cids)->orderBy('id')->get() as $r) {
        $lastRem[(int) $r->client_id] = ['at' => $r->created_at, 'by' => Db::adminName((int) $r->admin_id),
            'channel' => (string) $r->channel, 'amount' => (float) $r->amount, 'ticket' => (int) $r->ticket_id];
    }
    $lastNot = [];
    foreach (Capsule::table('mod_cpm_suspend_notices')->whereIn('client_id', $cids)->orderBy('id')->get() as $n) {
        $lastNot[(int) $n->client_id] = ['at' => $n->created_at, 'date' => $n->suspend_date, 'by' => Db::adminName((int) $n->admin_id)];
    }
    /* Υπηρεσίες σε κίνδυνο: ενεργές υπηρεσίες του πελάτη με module (το WHMCS τις
       αναστέλλει μόνο του) ή με ληγμένη επόμενη πληρωμή. */
    $svcBy = [];
    foreach (Capsule::table('tblhosting as h')->join('tblproducts as p', 'p.id', '=', 'h.packageid')
                 ->whereIn('h.userid', $cids)->whereIn('h.domainstatus', ['Active', 'Suspended'])
                 ->get(['h.id', 'h.userid', 'h.domain', 'h.domainstatus', 'h.nextduedate', 'h.amount', 'h.billingcycle',
                     'h.overideautosuspend', 'h.overidesuspenduntil', 'p.name as pname', 'p.servertype']) as $h) {
        $exempt = !empty($h->overideautosuspend) && ((string) $h->overidesuspenduntil === '' || strpos((string) $h->overidesuspenduntil, '0000') === 0
            || strtotime((string) $h->overidesuspenduntil) >= strtotime('today'));
        $svcBy[(int) $h->userid][] = ['id' => (int) $h->id, 'label' => (string) ($h->domain ?: $h->pname), 'product' => (string) $h->pname,
            'status' => (string) $h->domainstatus, 'auto' => (string) $h->servertype !== '', 'module' => (string) $h->servertype,
            'nextDue' => (string) $h->nextduedate, 'amount' => (float) $h->amount, 'cycle' => (string) $h->billingcycle, 'exempt' => $exempt];
    }
    $rows = [];
    $sumRisk = 0; $sumNotDue = 0;
    foreach ($bal as $c => $b) {
        $cl = $clients[$c] ?? null;
        $svcs = $svcBy[$c] ?? [];
        $active = array_values(array_filter($svcs, function ($x) { return $x['status'] === 'Active' && !$x['exempt']; }));
        $suspDate = $b['oldestDue'] ? date('Y-m-d', strtotime($b['oldestDue'] . ' +' . $graceB . ' days')) : null;
        $risk = $b['overdue'] > 0.5 && $active && $b['days'] >= max(0, $graceB - 5);   // κοντά ή πέρα από το όριο
        $rows[] = ['client' => $c, 'name' => html_entity_decode(clientLabel($c), ENT_QUOTES, 'UTF-8'), 'email' => $cl ? (string) $cl->email : '',
            'lang' => $cl ? cnp_client_lang($cl) : 'el', 'clientStatus' => $cl ? (string) $cl->status : '',
            'open' => $b['open'], 'overdue' => $b['overdue'], 'notDue' => round($b['open'] - $b['overdue'], 2),
            'days' => $b['days'], 'badDue' => $b['badDue'], 'ripe' => $b['days'] >= $graceB, 'risk' => $risk,
            'suspendDate' => $suspDate, 'invs' => $b['invs'], 'services' => $svcs,
            'activeServices' => count($active), 'autoServices' => count(array_filter($active, function ($x) { return $x['auto']; })),
            'reminded' => $lastRem[$c] ?? null, 'notified' => $lastNot[$c] ?? null,
            'adminUrl' => '/cloudonadminpanel/clientssummary.php?userid=' . $c];
        if ($risk) { $sumRisk++; }
        if ($b['overdue'] <= 0.5) { $sumNotDue++; }
    }
    /* Πρώτα όσοι κινδυνεύουν, μετά όσοι χρωστούν περισσότερα. */
    usort($rows, function ($a, $b) {
        return ($b['risk'] <=> $a['risk']) ?: ($b['overdue'] <=> $a['overdue']) ?: ($b['open'] <=> $a['open']);
    });
    out(['rows' => $rows, 'grace' => $graceB, 'sum' => ['clients' => count($rows),
        'open' => round(array_sum(array_column($rows, 'open')), 2),
        'overdue' => round(array_sum(array_column($rows, 'overdue')), 2),
        'risk' => $sumRisk, 'notDue' => $sumNotDue]]);

case 'balance_reminder':                 // σύνθεση υπενθύμισης εξόφλησης (πάγιο κείμενο ή AI)
    $cid = (int) ($in['client'] ?? 0);
    $cl = $cid ? Capsule::table('tblclients')->where('id', $cid)->first() : null;
    if (!$cl) { fail('client', 404); }
    $bal = cnp_open_balances($cid)[$cid] ?? null;
    if (!$bal) { fail('Ο πελάτης δεν έχει ανοιχτό υπόλοιπο'); }
    $pick = array_map('intval', (array) ($in['invoices'] ?? []));
    $invs = array_values(array_filter($bal['invs'], function ($x) use ($pick) { return !$pick || in_array($x['id'], $pick, true); }));
    if (!$invs) { fail('Δεν επιλέχθηκε παραστατικό'); }
    $graceB = (int) (Capsule::table('tblconfiguration')->where('setting', 'AutoSuspensionDays')->value('value') ?: 10);
    $lang = cnp_client_lang($cl, (string) ($in['lang'] ?? ''));
    $money = function ($v) { return number_format((float) $v, 2, ',', '.') . ' €'; };
    $name = trim(($cl->companyname ?: ($cl->firstname . ' ' . $cl->lastname)));
    $total = 0.0; $overdue = 0.0; $oldest = null;
    $invList = ''; $i9 = 0;
    foreach ($invs as $x) {
        $i9++; $total += $x['open'];
        if ($x['days'] > 0) { $overdue += $x['open']; if ($oldest === null || strtotime($x['due']) < strtotime($oldest)) { $oldest = $x['due']; } }
        $link = 'https://my.cloudon.gr/viewinvoice.php?id=' . $x['id'];
        if ($lang === 'el') {
            $late = $x['days'] > 0 ? ' · Καθυστέρηση: ' . $x['days'] . ($x['days'] === 1 ? ' ημέρα' : ' ημέρες') : ' · Δεν έχει λήξει ακόμη';
            $invList .= $i9 . ') Παραστατικό ' . $x['num'] . "\n   Έκδοση: " . cnp_d($x['issued']) . ' · Λήξη: ' . cnp_d($x['due']) . $late . "\n"
                . '   Αξία: ' . $money($x['gross']) . ($x['paid'] > 0.005 ? ' · Έχουν εξοφληθεί: ' . $money($x['paid']) : '')
                . ' · Υπόλοιπο: ' . $money($x['open']) . "\n   Προβολή & πληρωμή: " . $link . "\n";
        } else {
            $late = $x['days'] > 0 ? ' · Overdue by: ' . $x['days'] . ($x['days'] === 1 ? ' day' : ' days') : ' · Not yet due';
            $invList .= $i9 . ') Invoice ' . $x['num'] . "\n   Issued: " . cnp_d($x['issued']) . ' · Due: ' . cnp_d($x['due']) . $late . "\n"
                . '   Amount: ' . $money($x['gross']) . ($x['paid'] > 0.005 ? ' · Paid so far: ' . $money($x['paid']) : '')
                . ' · Outstanding: ' . $money($x['open']) . "\n   View & pay: " . $link . "\n";
        }
    }
    $invList .= ($lang === 'el' ? '   ΣΥΝΟΛΟ ΑΝΟΙΧΤΟΥ ΥΠΟΛΟΙΠΟΥ: ' : '   TOTAL OUTSTANDING: ') . $money($total) . "\n";
    $suspDate = $oldest ? date('Y-m-d', strtotime($oldest . ' +' . $graceB . ' days')) : null;
    $pastDue = $suspDate && strtotime($suspDate) < strtotime('today');   // το όριο έχει ήδη περάσει
    if ($pastDue) { $suspDate = date('Y-m-d'); }

    if ($lang === 'el') {
        $subject = 'Υπενθύμιση εξόφλησης — ανοιχτό υπόλοιπο ' . $money($total);
        $body = "Αγαπητοί συνεργάτες,\n\n"
            . "σας υπενθυμίζουμε ότι στον λογαριασμό σας υπάρχει ανοιχτό υπόλοιπο " . $money($total)
            . ($overdue > 0.5 && $overdue < $total - 0.5 ? ', εκ των οποίων ' . $money($overdue) . ' είναι ληξιπρόθεσμα' : '') . ".\n\n"
            . "ΑΝΑΛΥΣΗ ΠΑΡΑΣΤΑΤΙΚΩΝ\n" . $invList . "\n"
            . ($overdue > 0.5
            ...
            . "Αν η πληρωμή έχει ήδη γίνει, παρακαλούμε αγνοήστε το μήνυμα ή στείλτε μας το αποδεικτικό για να το αντιστοιχίσουμε.\n";
    } else {
        $subject = 'Payment reminder — outstanding balance ' . $money($total);
        $body = "Dear partners,\n\n"
            . "this is a friendly reminder that your account has an outstanding balance of " . $money($total)
            . ($overdue > 0.5 && $overdue < $total - 0.5 ? ', of which ' . $money($overdue) . ' is overdue' : '') . ".\n\n"
            . "INVOICES — BREAKDOWN\n" . $invList . "\n"
            . ($overdue > 0.5
    ...
    if (($in['mode'] ?? '') === 'ai') {
        $key = (string) (Capsule::table('tbladdonmodules')->where('module', 'cloudonprojects')->where('setting', 'ai_api_key')->value('value') ?: '');
        if ($key === '') { fail('Δεν έχει οριστεί AI API key — βάλ\' το στις Ρυθμίσεις'); }
        $draft = trim((string) ($in['draft'] ?? ''));
        $prompt = "Είσαι υπεύθυνος λογιστηρίου ελληνικής εταιρείας IT/hosting (CloudOn). Γράψε μια ΕΥΓΕΝΙΚΗ υπενθύμιση "
            . "εξόφλησης προς πελάτη — συνεργάτη, όχι οφειλέτη. Στόχος: να πληρώσει έγκαιρα ώστε να μη διακοπούν οι υπηρεσίες του.\n\n"
            . "ΓΛΩΣΣΑ: " . ($lang === 'el' ? 'ελληνικά' : 'αγγλικά') . " — αποκλειστικά.\n\n"
            . "ΚΑΝΟΝΕΣ — ΚΡΙΣΙΜΟ:\n- Χρησιμοποίησε ΑΚΡΙΒΩΣ τα ποσά, ημερομηνίες, ονόματα και συνδέσμους που δίνονται. Μην προσθέσεις δικά σου νούμερα, εκπτώσεις ή διακανονισμούς.\n"
            . "- Δομή: σύντομο μήνυμα, μετά ΑΝΑΛΥΣΗ ΠΑΡΑΣΤΑΤΙΚΩΝ αντιγραμμένη ΑΥΤΟΥΣΙΑ (με τους συνδέσμους), μετά πώς πληρώνει και τι γίνεται αν δεν πληρώσει.\n"
            . "- ΜΗΝ γράψεις θέμα/Subject. ΜΗΝ κλείσεις με υπογραφή — μπαίνει αυτόματα. Σκέτο κείμενο, χωρίς markdown.\n"
            . "- Τόνος: ζεστός, επαγγελματικός, χωρίς απειλές.\n\n"
            . ($draft !== '' ? "ΤΙ ΘΕΛΕΙ ΝΑ ΠΕΙ Ο ΣΥΝΑΔΕΛΦΟΣ (πες το αυτό, μη προσθέσεις άλλα):\n" . mb_substr($draft, 0, 1500) . "\n\n" : '')
            . "ΠΕΛΑΤΗΣ: " . $name . "\nΣΥΝΟΛΟ: " . $money($total) . "\nΛΗΞΙΠΡΟΘΕΣΜΑ: " . $money($overdue) . "\n"
            . ($pastDue ? "ΠΡΟΘΕΣΜΙΑ: έχει ήδη παρέλθει — ζήτα άμεση εξόφληση\n" : ($suspDate ? "ΠΡΟΘΕΣΜΙΑ ΠΡΙΝ ΤΗΝ ΑΝΑΣΤΟΛΗ: " . cnp_d($suspDate) . "\n" : ''))
            . "ΠΛΗΡΩΜΗ: https://my.cloudon.gr (τραπεζική κατάθεση, κάρτα ή PayPal)\n\nΠΑΡΑΣΤΑΤΙΚΑ:\n" . $invList . "\nΜΗΝΥΜΑ:";
        $resA = cnp_anthropic($key, 'claude-haiku-4-5-20251001', [['type' => 'text', 'text' => $prompt]], 1400);
        $txt = trim((string) ($resA['text'] ?? ''));
        if ($txt !== '') {
            $txt = preg_replace('/^\s*(Θέμα|Subject)\s*:.*\R+/ui', '', $txt);
            $txt = preg_replace('/\R+\s*(Με εκτίμηση|Με τιμή|Best regards|Kind regards|Sincerely)\s*,?\s*\R.*$/ui', '', $txt);
            $body = trim($txt) . "\n";
        }
    }
    $lastB = Capsule::table('mod_cpm_pay_reminders')->where('client_id', $cid)->orderBy('id', 'desc')->first();
    out(['subject' => $subject, 'body' => $body, 'lang' => $lang, 'total' => round($total, 2), 'overdue' => round($overdue, 2),
        'suspendDate' => $suspDate, 'invoices' => $invs, 'email' => (string) $cl->email, 'name' => $name,
        'lastReminder' => $lastB ? ['at' => $lastB->created_at, 'by' => Db::adminName((int) $lastB->admin_id), 'channel' => $lastB->channel] : null]);

case 'balance_reminder_send':            // αποστολή: email (καταγράφεται στο WHMCS) ή ticket
    $cid = (int) ($in['client'] ?? 0);
    $cl = $cid ? Capsule::table('tblclients')->where('id', $cid)->first() : null;
    if (!$cl) { fail('client', 404); }
    ...
    if (!filter_var((string) $cl->email, FILTER_VALIDATE_EMAIL)) { fail('Ο πελάτης δεν έχει έγκυρο email στο WHMCS — διόρθωσέ το πρώτα'); }
    $channel = (($in['channel'] ?? 'email') === 'ticket') ? 'ticket' : 'email';
    $bal = cnp_open_balances($cid)[$cid] ?? null;
    $amountB = $bal ? $bal['open'] : 0;
    $ticketId = 0; $tid = '';
    if ($channel === 'ticket') {
        ...
        $ticketId = (int) ($r['id'] ?? 0); $tid = (string) ($r['tid'] ?? '');
    } else {
        /* Email μέσω WHMCS (SendEmail → καταγράφεται στο Email Log του πελάτη). Το
           σκέτο κείμενο γίνεται HTML: οι σύνδεσμοι κλικάρονται, οι αλλαγές γραμμής μένουν. */
        $html = nl2br(preg_replace('~(https?://[^\s<]+)~u', '<a href="$1">$1</a>', htmlspecialchars($body, ENT_QUOTES, 'UTF-8')));
        $adm = Capsule::table('tbladmins')->where('id', $adminId)->first(['firstname', 'lastname']);
        $sig = ($cnpLangB = cnp_client_lang($cl, (string) ($in['lang'] ?? ''))) === 'el' ? 'Με εκτίμηση' : 'Kind regards';
        $msg = '<div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#243447;line-height:1.6;max-width:680px">'
            . $html . '<br><br>' . $sig . ',<br>' . htmlspecialchars(trim(($adm->firstname ?? '') . ' ' . ($adm->lastname ?? ''))) . '<br>CloudOn — Λογιστήριο</div>';
        $r = localAPI('SendEmail', ['customtype' => 'general', 'id' => $cid, 'customsubject' => $subject, 'custommessage' => $msg], 'pdelis');
        if (($r['result'] ?? '') !== 'success') { fail('Το email δεν στάλθηκε: ' . ($r['message'] ?? 'άγνωστο σφάλμα')); }
    }
    Capsule::table('mod_cpm_pay_reminders')->insert([
        'client_id' => $cid, 'invoices' => implode(',', array_map('intval', (array) ($in['invoices'] ?? []))),
        'amount' => round((float) $amountB, 2), 'channel' => $channel, 'ticket_id' => $ticketId ?: null,
        'admin_id' => $adminId, 'created_at' => date('Y-m-d H:i:s'),
    ]);
    logActivity('CloudOn PM: υπενθύμιση εξόφλησης προς πελάτη #' . $cid . ' (' . number_format((float) $amountB, 2, ',', '.') . ' €) μέσω '
        . ($channel === 'ticket' ? 'ticket ' . $tid : 'email ' . $cl->email) . ' από ' . Db::adminName($adminId));
    out(['ok' => true, 'channel' => $channel, 'ticket' => $ticketId, 'tid' => $tid, 'email' => (string) $cl->email]);
