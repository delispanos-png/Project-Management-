  ...
  function pane() {
    ...
    if (st.tab === 'setup') {
      ...
      { const bs = $('#phBaseSave', el); if (bs) bs.onclick = async () => {
        /* ΠΑΓΙΔΑ: αν ο διαχειριστής άνοιξε ΠΑΛΙΑ προσφορά, το st.cfg.r κρατά τις τιμές
           ΕΚΕΙΝΗΣ της προσφοράς — και το κουμπί θα τις έκανε σιωπηλά βασικές για όλες
           τις νέες. Γι' αυτό δείχνουμε ΓΡΑΜΜΗ-ΓΡΑΜΜΗ τι αλλάζει πριν αποθηκεύσουμε. */
        const base = (PH && PH.defaults && PH.defaults.r) || {};
        const eur = v => (Math.round(v * 100) / 100).toLocaleString('el-GR');
        const chg = (defs.rates || []).reduce((a, d) => {
          const n = +st.cfg.r[d.cell], o = +base[d.cell];
          if (isFinite(n) && isFinite(o) && Math.abs(n - o) > 0.005) {
            a.push('• ' + d.lab + ': ' + eur(o) + ' € → ' + eur(n) + ' €');
          }
          if (d.adj) {
            const na = +(st.cfg.r[d.adj] || 0), oa = +(base[d.adj] || 0);
            if (Math.abs(na - oa) > 0.0005) {
              a.push('• ' + d.lab + ' — ετήσια αναπροσαρμογή: '
                + Math.round(oa * 1000) / 10 + '% → ' + Math.round(na * 1000) / 10 + '%');
            }
          }
          return a;
        }, []);
        const body = 'Να γίνουν οι ΤΡΕΧΟΥΣΕΣ τιμές ΚΑΙ οι εκπτώσεις (CloudOn / υπηρεσιών / Soft1) '
          + 'ο βασικός τιμοκατάλογος για ΟΛΕΣ τις νέες προσφορές;\n\n'
          + (chg.length
              ? 'Αλλάζουν ' + chg.length + ' γραμμές σε σχέση με τον σημερινό βασικό:\n'
                + chg.slice(0, 14).join('\n') + (chg.length > 14 ? '\n• …και άλλες ' + (chg.length - 14) : '')
              : 'Καμία τιμή δεν αλλάζει σε σχέση με τον σημερινό βασικό — μόνο οι εκπτώσεις.')
          + '\n\nΟι ήδη αποθηκευμένες προσφορές ΔΕΝ αλλάζουν.';
        if (!(await window.CNP.cnpConfirm(body, {ok: '💾 Αποθήκευση', cancel: 'Άκυρο'}))) { return; }
        bs.disabled = true;
